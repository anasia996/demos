...
import { O as repeat, d as elapsedTicksAbsolute, f as frequencyTimer, g as relative, h as ofTotalTicks, m as ofTotal, r as StateMachineWithEvents, u as elapsedMillisecondsAbsolute, y as timerWithFunction } from "./src-_c_wzH5T.js";
...
const line = (bend = 0, warp = 0) => {
	...
	let cubicA = interpolate$2(Math.abs(bend), Empty, cubicB);
	if (bend !== 0 && warp > 0) if (bend > 0) cubicA = interpolate$2(warp, cubicA, {
		x: 0,
		y: cubicB.x * 2
	});
	else cubicA = interpolate$2(warp, cubicA, {
		x: cubicB.y * 2,
		y: 0
	});
...
function interpolatorAngleRadian(a, b, options) {
	...
	return (amount) => {
		let angle = a + interpolate$1(amount, 0, arc);
...
* @returns Interpolator function.
*/
function interpolatorInterval(duration, a = 0, b = 1, options) {
	...
	return (retargetB, retargetA) => {
		...
		if (retargetA !== void 0) a = retargetA;
		if (amount >= 1) return b;
		return interpolate(amount, a, b, options);
	};
}
...
function interpolatorCentered(a, b, options = byCharacter()) {
	if (typeof a !== `string`) throw new TypeError(`Param 'a' is not a string. Got: ${typeof a}`);
	if (typeof b !== `string`) throw new TypeError(`Param 'b' is not a string. Got: ${typeof b}`);
...
function interpolatorHuman(a, b) {
...
function interpolatorObject(startingValue, handlerFactory, options = {}) {
	...
	const createHandlers = () => {
		...
		for (const key in startingValue) {
			...
			else if (typeof value === `number`) handlers[key] = interpolate(a[key], b[key], options.optionsNumbers);
	...
	const interpolate$3 = (progression, newB, newA) => {
		retarget(newB, newA);
		const result = {};
		for (const key in b) {
...
*/
const mixModulators = (balance, a, b) => (amt) => interpolate$1(balance, a(amt), b(amt));
/**
...
var src_exports = /* @__PURE__ */ __exportAll({
	...
	interpolatorCentered: () => interpolatorCentered,
	interpolatorHuman: () => interpolatorHuman,
...
//# sourceMappingURL=src-CA4npwMN.js.map