...
* @param defaultValue
*/
function integerParse(value, range = ``, defaultValue = NaN) {
...
*/
function numberTest(value, range = ``, parameterName = `?`, info) {
...
* @returns
*/
function numberDecimalTest(a, b, decimals = 3) {
	if (decimals === 0) {
		a = Math.floor(a);
		b = Math.floor(b);
		if (a === b) return {
...
*/
function percentTest(value, parameterName = `?`, info) {
...
* @param parameterName Param name for customising exception message
*/
function integerTest(value, range = ``, parameterName = `?`) {
...
function integerArrayTest(numbers) {
...
function numberInclusiveRangeTest(value, min, max, parameterName = `?`) {
	if (typeof value !== `number`) return {
		success: false,
...
* @param parameterName Name of parameter (for more helpful exception messages)
*/
function equalWithPrecisionTest(expected, got, precision = 21, parameterName = `?`) {
...
//#endregion
//#region ../packages/guards/src/boolean.ts
/**
* Throws an error if parameter is not an boolean
* @param value
* @param parameterName
*/
function booleanTest(value, parameterName = `?`) {
	if (typeof value !== `boolean`) return {
		success: false,
		error: `Param '${parameterName} is not type boolean. Got: ${typeof value}`
	};
	return {
		success: true,
		value
...
function functionTest(value, parameterName = `?`) {
...
//#endregion
export { resultFirstFail_ as A, numberInclusiveRangeTest as C, errorResult as D, IxfxError as E, resultToError as F, resultToValue as I, resultWithFail as L, resultIsOk as M, resultThrow as N, getErrorMessage as O, resultThrowSingle as P, resultsCollate as R, numberDecimalTest as S, percentTest as T, integerArrayTest as _, testPlainObject as a, isInteger as b, isFunction as c, booleanTest as d, arrayIndexTest as f, ifNaN as g, equalWithPrecisionTest as h, rangeTest as i, resultIsError as j, resultErrorToString as k, isDefined as l, arrayTest as m, stringTest as n, testPlainObjectOrPrimitive as o, arrayStringsTest as p, rangeIntegerTest as r, functionTest as s, src_exports as t, nullUndefTest as u, integerParse as v, numberTest as w, isPowerOfTwo as x, integerTest as y, throwIfFailed as z };
//# sourceMappingURL=src-DZFdrMH_.js.map