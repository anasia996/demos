...
import { N as resultThrow, o as testPlainObjectOrPrimitive, u as nullUndefTest, w as numberTest, y as integerTest } from "./src-DZFdrMH_.js";
...
const updateByPath = (target, path, value, allowShapeChange = false) => {
	...
	return updateByPathImpl(target, path.split(`.`), value, allowShapeChange);
};
const updateByPathImpl = (o, split, value, allowShapeChange) => {
	...
	if (isInteger(start) && Array.isArray(o)) {
...
const getField = (object, path, separator = `.`) => {
	...
	return getFieldImpl(object, path.split(separator), path);
};
const getFieldImpl = (object, split, position) => {
	...
	if (isInteger(start) && Array.isArray(object)) {
		const index = Number.parseInt(start);
...
* @returns
*/
function* getPaths(object, options = {}) {
...
function* getPathsAndDataImpl(o, prefix, onlyLeaves = false, maxDepth) {
	...
	for (const entries of Object.entries(o)) {
		...
		if (onlyLeaves && typeof value !== `object` || !onlyLeaves) yield {
			path: sub,
			value
		};
...
//#endregion
//#region ../packages/core/src/trackers/track-unique.ts
/**
...
//#endregion
//#region ../packages/core/src/trackers.ts
var trackers_exports = /* @__PURE__ */ __exportAll({
	unique: () => unique,
...
*
* @param options Options to control the elapsed timer. Currently only supports `skip` which will skip the first N calls to the returned function before starting the timer.
* @returns Since
*/
function elapsedSince(options = {}) {
	let skip = options.skip ?? 0;
	let start = skip === 0 ? performance.now() : NaN;
	const fn = () => {
		if (skip > 0 && Number.isNaN(start)) {
			skip--;
			if (skip === 0) start = performance.now() - skip;
			else return NaN;
		}
...
* ```
*
* Use {@link isInfiniteSince} to check if a function is of type `infinity`:
* ```js
* let clicked = elapsedInfinity();
*
* document.addEventListener('click', () => {
...
* ```
*
* @returns A function that returns `Number.POSITIVE_INFINITY` when called.
*/
...
/**
* Returns _true_ if `fn` appears to be the {@link Since} type, and is of type `infinity`.
* Returns _false_ if `fn` is not a {@link Since} type, or is of a different type.
* @param fn
* @returns
*/
function isInfiniteSince(fn) {
	if (typeof fn !== `function`) return false;
	if (isSince(fn)) return fn._type === `infinity`;
	return false;
}
/**
* Returns _true_ if `fn` appears to be the {@link Since} type.
* @param fn
* @returns Boolean indicating if `fn` is a {@link Since} type.
*/
function isSince(fn) {
	if (typeof fn !== `function`) return false;
	if (typeof fn._type !== `string`) return false;
	return [
		`since`,
		`once`,
		`infinity`,
...
//#endregion
//#region ../packages/core/src/text-edit-distance.ts
/**
...
function levenshteinOps(a, b) {
	...
	for (let i = 0; i <= a.length; i++) {
		dp[i] = [];
		for (let j = 0; j <= b.length; j++) if (i === 0) dp[i][j] = j;
	...
	let i = a.length;
	let j = b.length;
	while (i > 0 || j > 0) if (i > 0 && j > 0 && dp[i][j] === dp[i - 1][j - 1] && a[i - 1] === b[j - 1]) {
		ops.push({
			type: `keep`,
			char: a[i - 1]
		});
...
//#endregion
//#region ../packages/core/src/text-tokenise.ts
const byWordRegex = /\s+/;
...
* @returns
*/
function abbreviate(source, maxLength = 15) {
...
* @returns
*/
function toStringAbbreviate(source, maxLength = 20) {
...
*/
function indexOfCharCode(source, code, start = 0, end = source.length - 1) {
...
* @returns
*/
function omitChars(source, removeStart, removeLength) {
	return source.slice(0, removeStart) + source.slice(removeStart + removeLength);
}
/**
...
*/
function* chunkByLength(source, size) {
	...
	if (pos < source.length) yield source.slice(pos);
}
/**
...
* @returns
*/
function beforeMatch(source, match, options = {}) {
	return beforeAfterMatch(source, match, options)[0];
}
/**
...
* @returns
*/
function afterMatch(source, match, options = {}) {
	return beforeAfterMatch(source, match, options)[1];
}
/**
...
* @returns
*/
function beforeAfterMatch(source, match, options = {}) {
	...
	const m = options.fromEnd ?? false ? source.lastIndexOf(match, startPos) : source.indexOf(match, startPos);
...
function wildcard(pattern) {
	const escapeRegex = (value) => value.replaceAll(escapeRegexPattern, `\\$1`);
...
function sorterByValueIndex(index, reverse = false) {
	return (values) => {
...
function keyValueSorter(sortStyle) {
	switch (sortStyle) {
...
const elapsedToHumanString = (millisOrFunction, rounding = 2) => {
	let interval = 0;
	if (typeof millisOrFunction === `function`) return elapsedToHumanString(millisOrFunction());
	else if (typeof millisOrFunction === `number`) interval = millisOrFunction;
	else if (typeof millisOrFunction === `object`) interval = intervalToMs(millisOrFunction);
...
async function resolve(resolvable, ...args) {
	...
	else if (typeof resolvable === `function`) return await resolvable(...args);
	else return resolvable;
}
/**
...
async function resolveFields(object) {
	...
	const entries = zip(keys, await Promise.all(resolvers));
	return Object.fromEntries(entries);
}
/**
...
//#endregion
//#region ../packages/core/src/url-parameters.ts
/**
...
function intParamOrDefault(parameters, name, defaultValue) {
	const p = parameters.get(name);
	if (p !== null) return Number.parseInt(p);
	return defaultValue;
}
/**
...
function floatParamOrDefault(parameters, name, defaultValue) {
	const p = parameters.get(name);
	if (p !== null) return Number.parseFloat(p);
	return defaultValue;
}
/**
...
//#endregion
export { addValue as $, wildcard as A, isEqualValueIgnoreOrder as At, isSince as B, toStringOrdered as Bt, beforeMatch as C, toObject as Ct, stringSegmentsWholeToEnd as D, isEqualContextString as Dt, omitChars as E, isEmptyEntries as Et, elapsedInfinity as F, numericComparer as Ft, trackers_exports as G, compareArrays as Gt, align as H, isPrimitive as Ht, elapsedInterval as I, defaultToString as It, getField as J, uniqueInstances as K, cloneFromFields as Kt, elapsedOnce as L, isMap as Lt, byWord as M, comparerInverse as Mt, levenshteinOps as N, defaultComparer as Nt, stringSegmentsWholeToFirst as O, isEqualDefault as Ot, filterValue as P, jsComparer as Pt, addObjectEntriesMutate as Q, elapsedSince as R, isSet as Rt, afterMatch as S, toArray as St, indexOfCharCode as T, zipKeyValue as Tt, alignById as U, isPrimitiveOrObject as Ut, defaultKeyer as V, records_exports as Vt, continuously as W, mapObjectShallow as Wt, pathed_exports as X, getPathsAndData as Y, updateByPath as Z, isInterval as _, maps_exports as _t, resolveFieldsSync as a, findEntryByPredicate as at, isInteger as b, sortByValue as bt, resolveWithFallback as c, fromIterable as ct, isReactive as d, getOrGenerate as dt, addValueMutate as et, promiseFromEvent as f, getOrGenerateSync as ft, isDateObject as g, mapToObjectTransform as gt, intervalToMs as h, mapToArray as ht, resolveFields as i, findBySomeKey as it, byCharacter as j, isEqualValuePartial as jt, toStringAbbreviate as k, isEqualValueDefault as kt, resolveWithFallbackSync as l, fromObject as lt, elapsedToHumanString as m, hasKeyValue as mt, sleep as n, deleteByValueCompareMutate as nt, resolve as o, findEntryByValue as ot, runningiOS as p, hasAnyValue as pt, compareData as q, sleepWhile as r, filterValues as rt, resolveSync as s, findValue as st, parseUrlParameters as t, addValueMutator as tt, hasLast as u, getClosestIntegerKey as ut, keyValueSorter as v, mergeByKey as vt, chunkByLength as w, transformMap as wt, isEqualTrace as x, sortByValueProperty as xt, compareIterableValuesShallow as y, some as yt, isInfiniteSince as z, toStringDefault as zt };
//# sourceMappingURL=src-CHZXopuG.js.map